...
module.exports = grammar({
  ...
  rules: {
    ...
    _statements: ($) =>
      prec(1, seq(repeat(seq($._statement, $._terminator)), $._statement, optional($._terminator))),
    ...
    for_statement: ($) =>
      seq(
        ...
        optional(seq("in", field("value", repeat1($._literal)))),
        $._terminator,
        field("body", $.do_group),
      ),
    ...
    _c_variable_assignment: ($) =>
      seq(field("name", alias($._c_word, $.variable_name)), "=", field("value", $._c_expression)),
    _c_unary_expression: ($) =>
      prec(PREC.PREFIX, seq(field("operator", choice("++", "--")), $._c_expression_not_assignment)),
    _c_binary_expression: ($) => {
      ...
      return choice(
        ...table.map(([operator, precedence]) => {
          // @ts-expect-error
          return prec[operator === "**" ? "right" : "left"](
            precedence,
            seq(
    ...
    _c_parenthesized_expression: ($) => seq("(", commaSep1($._c_expression), ")"),
    ...
    do_group: ($) => seq("do", optional($._terminated_statement), "done"),
    ...
    function_definition: ($) =>
      prec.right(
        seq(
          ...
          field("body", choice($.compound_statement, $.subshell, $.test_command, $.if_statement)),
    ...
    compound_statement: ($) =>
      choice(
        seq("{", optional($._terminated_statement), token(prec(-1, "}"))),
        seq("((", repeat(seq($._arithmetic_expression, ",")), $._arithmetic_expression, "))"),
      ),
    ...
    pipeline: ($) =>
      prec.right(
        seq($._statement_not_pipeline, repeat1(seq(choice("|", "|&"), $._statement_not_pipeline))),
      ),
    ...
    variable_assignment: ($) =>
      seq(
        ...
        choice("=", "+="),
        field("value", choice($._literal, $.array, $._empty_value, alias($._comment_word, $.word))),
      ),
      ...
      seq(
        ...
        optional(
          choice(
            ...
            seq(field("redirect", repeat1($._redirect)), optional($._heredoc_expression)),
            $._heredoc_expression,
            $._heredoc_command,
          ),
        ),
    ...
    _heredoc_expression: ($) =>
      seq(field("operator", choice("||", "&&")), field("right", $._statement)),
    ...
    herestring_redirect: ($) =>
      prec.left(seq(field("descriptor", optional($.file_descriptor)), "<<<", $._literal)),
    ...
    unary_expression: ($) =>
      choice(
        prec(PREC.PREFIX, seq(field("operator", tokenLiterals(1, "++", "--")), $._expression)),
        prec(PREC.UNARY, seq(field("operator", tokenLiterals(1, "-", "+", "~")), $._expression)),
        prec.right(PREC.UNARY, seq(field("operator", "!"), $._expression)),
        prec.right(PREC.TEST, seq(field("operator", $.test_operator), $._expression)),
      ),

    postfix_expression: ($) =>
      prec(PREC.POSTFIX, seq($._expression, field("operator", choice("++", "--")))),
    ...
    _arithmetic_unary_expression: ($) =>
      choice(
        prec(
          PREC.PREFIX,
          seq(field("operator", tokenLiterals(1, "++", "--")), $._arithmetic_expression),
        ),
        prec(
          PREC.UNARY,
          seq(field("operator", tokenLiterals(1, "-", "+", "~")), $._arithmetic_expression),
        ),
        prec.right(PREC.UNARY, seq(field("operator", "!"), $._arithmetic_expression)),
      ),

    _arithmetic_postfix_expression: ($) =>
      prec(PREC.POSTFIX, seq($._arithmetic_expression, field("operator", choice("++", "--")))),
    ...
    _expansion_body: ($) =>
      choice(
        ...
        seq(
          ...
          optional(field("operator", choice(token.immediate("@"), token.immediate("*")))),
        ),
        seq(
    ...
    _expansion_regex: ($) =>
      seq(
        ...
        repeat(choice($.regex, alias(")", $.regex), $.string, $.raw_string, alias(/\s+/, $.regex))),
      ),
    ...
    _expansion_regex_removal: ($) =>
      seq(field("operator", choice(",", ",,", "^", "^^")), optional($.regex)),
    ...
    _comment_word: (_) =>
      token(
        prec(
          -8,
          seq(
            choice(noneOf(...SPECIAL_CHARACTERS), seq("\\", noneOf("\\s"))),
    ...
    _multiline_variable_name: ($) => alias(token(prec(-1, /(\w|\\\r?\n)+/)), $.variable_name),
