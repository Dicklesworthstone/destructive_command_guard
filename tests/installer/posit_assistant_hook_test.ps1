...
# unrelated settings/groups, stale-path repair, refuse-invalid-JSON, skip, and
# Detect-Agents wiring. The function takes a -HomeDir param so a temp home can be
# injected ($HOME is read-only in PowerShell).
#
# Three Posit-specific invariants are asserted deliberately, because getting any
# of them wrong produces a hook that is present in the file but never fires:
#   - the matcher is lowercase "bash|powershell" (a simple matcher string is an
#     exact match against the tool name, and both shell-tool names are covered);
#   - only documented handler fields are emitted, so there is NO `shell` field
#     (the Claude entry uses one; the command is quoted for cmd.exe instead);
...
try {
    ...
    Check ($hook.command -eq $expectedCommand) "dcg command is quoted for cmd.exe so a path with spaces resolves"
    Check ($hook.type -eq 'command') "hook type is 'command'"
    Check ($hook.timeout -eq 30) "timeout is 30 (seconds)"
    ...
    $existing = [ordered]@{
        hooks = [ordered]@{
            PreToolUse = @([ordered]@{
                ...
                hooks = @(
                    [ordered]@{ type = 'command'; command = '"C:\old\dcg.exe"'; timeout = 30 },
...
# Clear PATH so `pa` is not discoverable; with ~/.posit/assistant absent and no
# -Force the result must be 'skipped' and nothing may be written.
