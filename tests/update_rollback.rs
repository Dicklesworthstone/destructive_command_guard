...
fn test_rollback_to_specific_version() {
    ...
    create_mock_backup_entry(&backup_dir, "1.9.0", 1737100000, b"version 1.9.0");
...
fn test_rollback_to_most_recent_backup() {
    ...
    create_mock_backup_entry(&backup_dir, "0.2.10", 1737000000, b"v0.2.10");
    create_mock_backup_entry(&backup_dir, "0.2.11", 1737100000, b"v0.2.11");
    create_mock_backup_entry(&backup_dir, "0.2.12", 1737200000, b"v0.2.12");
    ...
    let content = fs::read(&binary_path).unwrap();
    assert_eq!(content, b"v0.2.12", "Should rollback to most recent (0.2.12)");
}
...
fn test_backup_pruning_keeps_max_backups() {
    ...
    create_mock_backup_entry(&backup_dir, "0.2.9", 1736900000, b"v0.2.9");
    create_mock_backup_entry(&backup_dir, "0.2.10", 1737000000, b"v0.2.10");
    create_mock_backup_entry(&backup_dir, "0.2.11", 1737100000, b"v0.2.11");
    create_mock_backup_entry(&backup_dir, "0.2.12", 1737200000, b"v0.2.12");
    create_mock_backup_entry(&backup_dir, "0.2.13", 1737300000, b"v0.2.13");
...
fn test_version_cache_structure() {
    ...
    let cache_content = serde_json::json!({
        ...
        "cached_at_secs": 1737200000
    });

    fs::write(&cache_path, serde_json::to_string_pretty(&cache_content).unwrap()).unwrap();
...
fn test_cache_expiration_check() {
    ...
    let cached_at: u64 = 1737200000;
    let now_fresh: u64 = 1737200000 + 3600; // 1 hour later
    let now_expired: u64 = 1737200000 + 100000; // > 24 hours later
