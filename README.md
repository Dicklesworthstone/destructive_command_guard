...
- **Posit Assistant:** [Posit Assistant](https://positron.posit.co/assistant/) reads Claude-Code-compatible lifecycle hooks from `~/.posit/assistant/settings.json` (global) and `<workspace>/.posit/assistant/settings.json` (project). The installer merges one `PreToolUse` entry into the **global** file, so a single install covers every workspace. No protocol work is needed on dcg's side: the documented `PreToolUse` stdin is the snake_case Claude shape (`tool_name`, `tool_input.command`, `tool_use_id`, `permission_mode`), exit code 2 blocks with stderr shown as the reason, and `hookSpecificOutput.permissionDecision` (`allow`/`deny`/`ask`) is read on exit 0 — so dcg reuses its existing Claude-compatible response. Three details differ from the Claude entry: the matcher is **lowercase** `"bash|powershell"` (a simple matcher string is an *exact* match, or a `|`/`,`-separated list of exact matches, against the tool name, so a copied Claude `"Bash"` matcher would not fire — and listing both names covers a PowerShell host with one entry); only documented handler fields are emitted, so unlike the Claude entry there is **no `shell` field** (the Windows command is quoted instead, since shell-form hooks run through `cmd.exe`); and `timeout` is in **seconds** (30, matching the documented example). dcg identifies the agent at runtime from `PA_PROJECT_DIR`, which the hook contract sets in the hook subprocess. Existing matcher groups are left structurally intact rather than consolidated — hook config is additive, so there is no need to rewrite a user's `matcher: "bash,edit"` group — and `unconfigure_posit_assistant` in `uninstall.sh` removes only dcg-owned entries, never the settings file, since unrelated settings live there too. Set `PA_DISABLE_HOOKS=1` before launching Posit Assistant to bypass all hooks, dcg's included.
