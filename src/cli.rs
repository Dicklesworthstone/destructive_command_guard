...
use crate::history::{ExportOptions, HistoryDb, HistoryStats, Outcome};
...
fn test_command(
    ...
    // Use color based on terminal detection and config
    let use_color = should_use_color(&effective_config.general);

    // Get terminal width for highlighting (default to 80 if unavailable)
